...
`check-changelog.sh` decides the mechanical half of all of this — the heading shapes, real dates, the ordering, a heading repeated, `Unreleased` where it does not belong or below a release, and whether the current `VERSION` has a section. The shapes above are the ones this skill writes; the checker also takes the other templates popular changelogs follow, listed in its `--help`, as long as one changelog keeps to one. What it cannot decide is whether an entry deserved to exist, which is the half that needs you
